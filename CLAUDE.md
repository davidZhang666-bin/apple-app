...
康源华善 (KYHSApp) — a SwiftUI iOS app for shop owners (店长) covering live-streaming center, video quizzes, and a user center with QR-code clock-in. Bundle ID `cn.kangyuanhuashan.app`, Swift 5, iOS 18 deployment target. UI strings are Chinese (zh_CN).

The Xcode project lives at `KYHSApp/KYHSApp.xcodeproj`. There are no Swift Package Manager / CocoaPods dependencies in the repo today — `wechat-login-plan.md` is a not-yet-executed plan to introduce CocoaPods for WeChat SDK; if you start that work, follow that doc and switch to `KYHSApp.xcworkspace`.

## Build / run

Open `KYHSApp/KYHSApp.xcodeproj` in Xcode and run on a simulator or device. From the CLI:
